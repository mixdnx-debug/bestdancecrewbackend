...
// ---- Seed data (Mella Awards 2026 — The Best Dance Crew in Chuka) ----
const SEED_VERSION = 'v2-2026-mella-awards';

const seedCategories = [
  { id: 'influencers-of-the-year', title: 'The Best Dance Crew in Chuka', nominees: [
    ['254 Dance Crew', ''],
    ['New Blyden Dance Crew', ''],
    ['Squash Dance Crew', ''],
    ['Viral Dance Crew', ''],
  ]},
];
...
if (catCount === 0) {
  ...
  db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('seed_version', SEED_VERSION);
    console.log('[db] Seeded Mella Awards 2026 categories and nominees.');
...
// ============================================================
//  Catalogue healer — guarantees the Best Dance Crew in Chuka
//  category ALWAYS contains exactly the 4 official nominees,
...
const CANONICAL_ROSTER = [
  { name: '254 Dance Crew',        aliases: [] },
  { name: 'New Blyden Dance Crew', aliases: [] },
  { name: 'Squash Dance Crew',     aliases: [] },
  { name: 'Viral Dance Crew',      aliases: [] },
];
...
function canonicaliseCatalogue() {
  try {
    ...
    const tx = db.transaction(() => {
      ...
      // Remove any row in this category that is NOT one of the official 4
    ...
    tx();
    if (changed) console.log('[db] Catalogue canonicalised — exact 4-nominee roster enforced.');
    return changed;
  } catch (e) {
